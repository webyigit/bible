...
앱이 매일 해당하는 영상을 자동으로 가져오게 할 수 있습니다.

**날짜 매칭 방식**: "단순히 최신 영상"이 아니라, **읽기 시작일(설정 > 성경 본문 소스 > 읽기
시작일, 기본값 올해 1월 1일) 기준으로 오늘이 며칠째인지 계산해서 재생목록의 그 순번(N번째)
영상을 가져옵니다.** 예를 들어 오늘이 읽기 시작일로부터 200일째면 재생목록의 200번째 영상을
가져오는 식입니다. 성경 본문(하루 1장)도 똑같은 날짜 세기를 쓰기 때문에, 재생목록이 1일차
영상부터 순서대로 쌓여있다면 영상과 본문이 항상 같은 날로 맞물려서 나옵니다.

재생목록의 1번째 영상이 실제로 "1일차"가 맞는지, 즉 읽기 시작일과 재생목록이 시작된 날짜가
같은지 꼭 확인해주세요. 어긋나 있다면 설정에서 읽기 시작일을 재생목록이 실제로 시작된
날짜로 맞춰주시면 됩니다.
...
10,000유닛이고, N번째 영상을 찾으려면 최대 50개씩 페이지를 넘겨가며 확인해야 하지만
하루 한 번만 계산하므로 충분합니다.
...
### bolls.life가 응답하지 않을 때를 대비한 안전장치

본문이 실시간 API 호출이라, bolls.life가 잠깐이라도 다운되거나 이 API 자체가
장기간 막히면 그 순간 앱에서 본문을 못 보여줍니다. 이를 대비해 두 겹의 안전장치를
뒀습니다.

1. **기기 캐시**: 한 번 성공적으로 받아온 장은 브라우저 로컬 저장소에도 그대로
   저장해둡니다. 다음에 같은 장을 열 때 실시간 호출이 실패해도 마지막으로 성공한
   내용을 그대로 보여줍니다(화면에 "실시간 서버에 연결이 안 돼 저장된 본문을
   보여드려요" 안내가 뜹니다). 기기별로 저장되므로 새 기기·새 브라우저에서는
   해당되지 않습니다.
2. **저장소 백업(`bible-backup/`)**: 이 저장소에 성경 전체(1,189장) 본문을 JSON
   파일로 미리 받아 함께 배포해둡니다. 기기 캐시도 없을 때(첫 실행 등) 최후
   수단으로 이 파일을 읽습니다. `.github/workflows/backup-bible-text.yml`이 매달
   1일 자동으로 bolls.life를 다시 긁어 이 폴더를 최신 상태로 갱신·커밋합니다
   (GitHub Actions 화면에서 **Backup Bible text** 워크플로를 수동으로도 실행할
   수 있습니다).

이 저장소를 만든 샌드박스는 외부망(bolls.life 포함)이 막혀 있어 `bible-backup/`을
직접 채우지는 못했습니다 — 위 워크플로가 GitHub의 실행 환경(인터넷 제한 없음)에서
돌면서 채우고 계속 최신화합니다. 처음 배포한 뒤 워크플로가 한 번은 돌아야 백업이
생깁니다.
